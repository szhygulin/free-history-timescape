...
%   (2) Machine-verify all arXiv ids vs live arXiv before submission;
%       Wiltshire2009review / WiegandBuchert2010 / ShethvdW2004 /
%       ClampittJain2015 carry arXiv-only (no volume/page asserted).
...
\begin{abstract}
Paper~I of this program showed that the timescape cosmology in its standard
one-parameter tracker closure cannot fit type~Ia supernovae (SNe), baryon
acoustic oscillations (BAO), and the cosmic microwave background (CMB) acoustic
scale together: the SNe drive the present void fraction toward $\fvo\simeq0.85$
while BAO$+$CMB demand $\simeq0.64$ (a $4.8$--$6.6\sigma$ split), yet a
model-independent free-$E(z)$ check shows the data are not internally
contradictory---the split is the tracker shape's rigidity, not a data conflict.
Here we free the void-fraction history $\fvz$ from the attractor, keeping the
Buchert two-phase average and the wall/void clock-rate dressing, and re-test
end to end. \emph{Part~I (kinematic):} the freed $\fvz$ reconciles Pantheon$+$,
DESI DR2 BAO, and the Planck acoustic scale at joint $\chi^2=1396.1$, below flat
$\Lcdm$ ($1402.2$) and far below the tracker ($1469.3$), at a physically
plausible $\fvo=0.640$; the amplitude split dissolves to $0.16\sigma$ (tracker:
$6.5\sigma$). But this is the backreaction the Hubble diagram \emph{wants}---a
kinematic target that violates Buchert integrability, not a proven solution.
\emph{Part~II:} four independent falsification tests each fail. The required
history must decline by $\times3.31$ over $0<z<2.33$ while the observed BOSS DR12
void population declines only $\times1.16$ (shape-unavailable, by a floor
theorem); the predicted local expansion-rate bias is $+2.4\%$ ($+1.6\%$
lensing-physical) versus the $+8.4\%$ SH0ES requires ($4.2\sigma$,
measurement-only); enforcing dynamical consistency collapses the three-phase
Buchert solve to the tracker limit, missing the geometry model-comparison bar by
$4953$; and the in-model sound horizon is $\rd=199.6$~Mpc ($+36\%$), driving the
global bare $\Hbaro$ to $\sim\!40\kmsmpc$. Removing the tracker's rigidity
reconciles the distance geometry, but the flexibility is kinematic: free-history
timescape does not resolve the Hubble tension. Consistent with Paper~I,
eliminating dark energy through inhomogeneity does not resolve the tension on
present public data.
\end{abstract}
...
Apparent acceleration follows without dark energy. Recent Pantheon$+$ reanalyses with a
cosmology-independent covariance report strong Bayesian evidence for timescape
over $\Lcdm$ \cite{Lane2025,Seifert2024}, reviving the question of whether the
framework also addresses the Hubble tension.
...
We state the two-part answer up front. \textbf{Part~I} (Sec.~\ref{sec:reconcile}):
freeing $\fvz$ \emph{does} reconcile the geometry. The freed history fits
Pantheon$+$, DESI DR2 BAO, and the Planck acoustic scale at joint $\chi^2=1396.1$,
below flat $\Lcdm$ ($1402.2$) and far below the tracker ($1469.3$), at a
physically plausible present void fraction $\fvo=0.640$; the SN-versus-BAO$+$CMB
amplitude split dissolves from the tracker's $6.5\sigma$ to $0.16\sigma$.
Throughout we label this reconciliation \emph{kinematic}: it is a well-defined
target the Hubble diagram wants, but the forced history need not satisfy the
Buchert integrability conditions and is not, on its face, a general-relativistic
solution. \textbf{Part~II} (Sec.~\ref{sec:partii}): four independent
falsification tests ask whether the reconciling history is physically
\emph{available}, dynamically \emph{consistent}, and early-Universe
\emph{calibratable}. All four fail. The required history is not present in the
observed void population (its decline is $\times3.31$ against an observed
$\times1.16$; \emph{shape-unavailable}); it predicts a local expansion-rate bias
of $+2.4\%$ against the $+8.4\%$ required, at $4.2\sigma$ (\emph{fails}); enforced
onto a dynamical three-phase Buchert solve it collapses to the two-phase tracker
limit and misses the geometry bar by $4953$ (\emph{closed}); and its in-model
sound horizon is $\rd=199.6$~Mpc, $+36\%$ above the external ruler, driving the
bare Hubble rate to $\sim\!40\kmsmpc$ (\emph{fails}). This is
\emph{reconciliation without resolution}: removing the tracker's rigidity
reconciles the distance geometry, but the flexibility is kinematic and closes at
every physical test. The result corroborates and sharpens Paper~I's conclusion.
...
error $1.7\times10^{-9}$ and an SN $\chi^2=1391.545$ (matching the Paper~I
...
fixed across models). The BAO data are the DESI DR2 distances \cite{DESIDR2};
the CMB enters only through the Planck 2018 acoustic-scale point
\cite{Planck2018}. As in Paper~I, the sound-horizon ruler $\rd$ enters BAO$+$CMB
...
($=\chi^2_{\Lcdm}+10$); \emph{disfavoured} if $\le1427.24$;
\emph{amplitude-dead} if $\ge1469.29$ (the tracker value).
...
$\chi^2\le1412.24$. Column $\chi^2-\chi^2_{\Lcdm}$ is the difference from the
$\Lcdm$ row.}
\label{tab:jointchi2}
\begin{ruledtabular}
\begin{tabular}{lcc}
Model & joint $\chi^2$ & $\chi^2-\chi^2_{\Lcdm}$\\
\colrule
free $E(z)$ spline (model-indep.)      & $1391.85$ & $-10.39$\\
free-history TS ($\fvz$) \emph{[this work]} & $1396.06$ & $-6.18$\\
$w_0w_a$CDM                            & $1398.29$ & $-3.95$\\
flat $\Lcdm$                           & $1402.24$ & $0$\\
timescape tracker (1-param)           & $1469.29$ & $+67.06$\\
\end{tabular}
\end{ruledtabular}
\footnotesize A no-lapse control (V0) gives $\chi^2=1396.49$, confirming the
result is not an artifact of the lapse convention. An independent multistart
re-derivation reproduces the headline $\chi^2=1396.06$ and cannot improve on it.
\end{table}
...
pre-registered verdict is \textbf{RECONCILES (mechanism flexible)}; an
independent re-derivation returns the same verdict.

The reconciliation is robust to the lapse convention. The LB (rate-ratio) sibling
fits at $\chi^2=1406.92$ ($\fvo=0.588$; $+4.69$ above $\Lcdm$, still inside the
...
The same statistic applied to the tracker as a sanity check reproduces Paper~I:
$\fvo^{\mathrm{SN}}=0.853$ against $\fvo^{\mathrm{BAO+CMB}}=0.636$,
$\Delta\chi^2=42.85$, a $\boldsymbol{6.5\sigma}$ split. The split
\emph{dissolves} once the history shape is free; an independent adversarial
re-derivation confirms the dissolution at $0.16\sigma$.
...
parametrisation's rigidity, not a data conflict.
...
solution. Part~II asks, at full detail, whether that target is available in the
...
verdict. All four fail; the honest negative result is the value of the paper.
...
literature \cite{Pan2012,Williams2024}.
...
$1.3$  & $0.279$ & $[0.361,0.499]$     & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} & FAIL: $r_{\mathrm{obs}}>r_{\mathrm{req}}^{\mathrm{hi}}$\\
$2.33$ & $0.194$ & $[0.231,0.363]$     & \multicolumn{1}{c}{---} & \multicolumn{1}{c}{---} & FAIL: $r_{\mathrm{obs}}>r_{\mathrm{req}}^{\mathrm{hi}}$\\
\end{tabular}
\end{ruledtabular}
...
$[0.560,0.654]$ across $R_s\in\{2,4,8\}\hmpc$, all $\ge0.5$ (floor holds). At
$z=1.3,2.33$ the observed ratio exceeds the required-hi edge at every node
(pre-registered SHAPE test).
\end{table*}

The observed void fraction declines by a total factor of only $\times1.16$ over
the same range---the population is far flatter than the required history. At the
...
undershoot: a \emph{floor theorem} on admissible void populations makes the
required steepness unreachable, formalised by the negative significance
$\sigma_{\mathrm{needed}}=-0.53$ that the observed value would need to move to the
required one (impossible). The pre-registered SHAPE test declares the history
...
\textbf{Verdict: SHAPE-UNAVAILABLE.} An independent re-derivation of the two-part
telescope-forced test survives all four checks. A fit-space corroboration
...
$\chi^2=2845.6$ on DR2, missing the (one-fewer-parameter) BIC bar \cite{KassRaftery1995} of $1407.2$ by
$1438$; the miss is $>1400$ under every covariance treatment tested, and an
independent re-derivation passes $42/42$ checks. The observed void population
supplies the required present-day level but categorically cannot supply the
required decline shape.
...
\emph{An in-programme derivation of the apparent-$\Ho$ window.} The maximum
apparent-$\Ho$ excess a wall observer can register at the void scale is
\begin{equation}
\Emax(\fvo)=\frac{3/2}{\gdress(\fvo)}-1,
\label{eq:emax}
\end{equation}
with $\gdress$ from Eq.~(\ref{eq:dressed}). This reproduces the Wiltshire
$17$--$22\%$ apparent-$\Ho$ window from first principles: $\Emax(0.76)=0.171$ and
$\Emax(0.695)=0.220$ (validation gate a, matching the literature window
\cite{Wiltshire2009,Wiltshire2009review,Wiltshire2013}). At the LA reading the
small-radius void-scale ceiling is $\Emax=0.3365$ (gate b; this corrects an
earlier mislabeling of the volume-average $0.195$ as ``the maximum'').

\emph{The SH0ES geometry dilutes the ceiling.} The second-rung Hubble-flow sample
does not sit at the void-scale maximum; averaging $\Emax$ over the ladder geometry
($n_{\mathrm{calib}}=77$ calibrators with mean void weight
$\langle\varphi\rangle_{\mathrm{calib}}=0.994$, $n_{\mathrm{HF}}=277$
Hubble-flow SNe with $\langle\varphi\rangle_{\mathrm{HF}}=0.071$) gives a
survey-averaged central
\begin{equation}
\bpred^{\,\mathrm{survey}}=0.0240,
\end{equation}
a $14\times$ dilution of the ceiling $\Emax=0.3365$. The lensing-\emph{physical}
void rate (calibrated to stacked void lensing densities \cite{ShethvdW2004,ClampittJain2015})
gives $\bpred^{\,\mathrm{phys}}=0.0158$, band $[0.0150,0.0287]$. The entire
admissible $\varphi$-band, $[0.0004,0.0720]$, lies \emph{below} $\breq=0.0842$: a
one-sided under-prediction.

\emph{Verdict against the pre-registered threshold.} The P3 threshold
\cite{Paper1} is $|\bpred-\breq|\le1\sigma\Rightarrow$ RESOLVES;
$\le2\sigma\Rightarrow$ PARTIAL; else FAILS. With
$\bpred^{\,\mathrm{survey}}-\breq=-0.0602$, the measurement-only significance is
$n\sigma_{\mathrm{meas}}=4.16$ (\textbf{FAILS}); the pre-registered significance,
which inflates $\sigma$ with the wide $\varphi$-shape \emph{theoretical}
systematic, is $n\sigma_{\mathrm{prereg}}=1.57$ (PARTIAL). We report both, and
the robust reading \textbf{FAILS}: the PARTIAL survives only because the dominant
$\varphi$-shape theoretical systematic inflates the denominator. Consistently,
the profile-decay scale needed to reach $\breq$ is
$r_{\mathrm{hom}}^{\ast}=140\hmpc$, outside the declared $[80,120]\hmpc$ range and
beyond the $\sim\!137.5\hmpc$ 2M$++$ homogeneity scale.
...
$\Emax^{\mathrm{obs}}=0.0658$ and $\bpred^{\,\mathrm{survey,obs}}=0.0047$, while
...
fitted target and $68.1\times$ against the (larger) catalog-forced target. An
independent re-derivation of both legs survives.

\textbf{Verdict: FAILS.} The predicted local bias is an order of magnitude below
the required $+8.4\%$ whether the void structure is fitted to the Hubble diagram
or supplied by the telescope. A FAILS verdict here kills the local-bias
resolution for this model family regardless of the joint-fit quality of Part~I.
...
integrability; here we quantify it and make the honest attempt to \emph{restore}
...
two-scale GR solution. Its fractional drift is $\sim\!152\%$ across the interior
of the forced required history and $\sim\!5\times$ (span) for the observed
history, against the tracker's $2\times10^{-6}$ pipeline floor---six-plus orders
of magnitude worse. An exact-analytic transcription check drifts only at machine
precision, so the drift is physical, not numerical. \emph{No consistent
single-phase Buchert model tracks either history.}
...
stage verdict is GO. But the best fit is \emph{degenerate}: the extra void
phase's amplitude drives to $\alpha_{s2}\to0$, i.e.\ the three-phase model
collapses to the two-phase empty-void tracker with a single genuine void of
fraction $f_{d0}\simeq0.27$. The tracking solution ``tracks'' only by becoming
the tracker.

The pre-registered geometry bar (DR2) is
$\chi^2\le\chi^2_{\Lcdm}+\ln N\approx1407.2$. The zero-parameter ($k=0$)
...
Even the never-a-claim \emph{constants-freed} diagnostic ceiling---which frees
parameters the theory does not permit---misses the bar by $107$
($\dBIC=+136$ versus $\Lcdm$).
...
ruler. WP-C replaces that crutch with the model's own bare-frame radiation-era
physics (DNW13 machinery \cite{Duley2013}) and asks what $\rd$ free-history
timescape actually predicts. The answer is
\begin{equation}
\rd^{\,\mathrm{in\text{-}model}}=199.6~\mathrm{Mpc},
\end{equation}
$+35.7\%$ above the external ruler ($r_{\mathrm{dec}}=195.9$~Mpc). The machinery
is validated: its $\Lcdm$ gate returns $r_\star=144.3$~Mpc and
$r_{\mathrm{drag}}=146.9$~Mpc, matching the expected values.

Because the acoustic-scale $\chi^2$ fixes an \emph{angle}, it is
$\rd$-independent, so Part~I's distance fit (Sec.~\ref{subsec:probeR}) is
untouched: the acoustic ratio $r_\star/r_{\mathrm{drag}}$ matches Planck to
$0.03\%$ ($0.9816$ vs $0.9819$), even though the absolute $\rd$ does not (the
in-model compression gives $\ell_A=218.9$ vs Planck's $301.7$, bare shift
$R=1.11$ vs $1.74$, and a BBN-$\eta$ baryon density $\omega_b=0.0186$ vs
$0.0224$). But the \emph{absolute} Hubble rate is not $\rd$-independent. With the
...
below both the reconciling band and SH0ES.

The pre-registered question is whether the Part~I \emph{absolute}-$\Ho$ claim
survives an in-model sound horizon; it does not
($R1_{\mathrm{survives}}=\mathrm{false}$). An independent re-derivation
reproduces every headline number. As in Paper~I, we distinguish resolution from
...
Table~\ref{tab:wpn} aggregates the program's tension ledger: for each tension,
the free-history (kinematic) status, whether it is \emph{self-inflicted} by the
...
$\breq=0.0842$ (meas.-only $n\sigma=4.16$ ceiling / $4.73$ physical) &
...
diagnosis was correct: the SN-versus-BAO$+$CMB split is the tracker's
one-parameter rigidity, and removing that rigidity by freeing $\fvz$ reconciles
the distance geometry below $\Lcdm$ at a plausible $\fvo=0.640$, with the
amplitude split collapsing from $6.5\sigma$ to $0.16\sigma$
(Sec.~\ref{sec:reconcile}). Yet the reconciling history is kinematic, and the
kinematic/dynamic gap is the whole story: the history is \emph{not available} in
the observed void population (Sec.~\ref{subsec:availability}; the required
$\times3.31$ decline against an observed $\times1.16$, unbridgeable by a floor
theorem), \emph{not a GR solution} (Sec.~\ref{subsec:dynamical}; the three-phase
Buchert attempt collapses to the tracker and misses the geometry bar by $4953$),
\emph{not early-Universe calibratable} (Sec.~\ref{subsec:soundhorizon}; an
in-model $\rd$ that is $+36\%$ too large drives the bare rate to $\sim\!40$), and
predicts \emph{the wrong local bias} (Sec.~\ref{subsec:bpred}; $+2.4\%$ against
the required $+8.4\%$, at $4.2\sigma$ measurement-only, and failing harder when
the observed catalog is forced in).
...
dissolving to $0.16\sigma$. But that flexibility is kinematic, and it closes at
every deeper physical test: availability (shape-unavailable, decline
$\times3.31$ required vs $\times1.16$ observed), local bias (fails at
$4.2\sigma$, $+2.4\%$ predicted vs $+8.4\%$ required), dynamical consistency
(closed; the three-phase Buchert solve collapses to the tracker and misses the
geometry bar by $4953$), and the sound horizon (fails; in-model $\rd=199.6$~Mpc,
$+36\%$, driving the bare rate to $\sim\!40\kmsmpc$). Reconciliation without
resolution. This corroborates and sharpens Paper~I: on present public data,
eliminating dark energy through inhomogeneity---whether through the rigid tracker
or its fully freed history---does not resolve the Hubble tension.
...
drafting. All sources, equations, code, and numerical results were verified by
the author, who takes full responsibility for the content and any errors. This
...
\cite{Carrick2015}. This repository, \texttt{free-history-timescape}, holds the
Part~I geometry-reconciliation code at the root and the Part~II tension-test code
under \texttt{tensions/}; the former \texttt{free-history-timescape-tensions}
repository is archived read-only on GitHub as the per-commit history of record.
The code is released under the MIT license and will be deposited publicly on
publication.
